...
export const snsApi = {
  ...
  // SNS 계정 연결
  connectAccount: (platform, data) =>
    api.post(`${BASE_URL}/accounts/${platform}`, data),

  // SNS 계정 연결 해제
  disconnectAccount: (platform) =>
    api.delete(`${BASE_URL}/accounts/${platform}`),

  // 예약 게시물 목록
  getScheduledPosts: (params) =>
    api.get(`${BASE_URL}/scheduled-posts`, { params }),
  ...
  updateScheduledPost: (postId, data) =>
    api.put(`${BASE_URL}/scheduled-posts/${postId}`, data),

  // 예약 게시물 삭제
  deleteScheduledPost: (postId) =>
    api.delete(`${BASE_URL}/scheduled-posts/${postId}`),
  ...
  getHashtagSuggestions: (keyword) =>
    api.get(`${BASE_URL}/hashtags`, { params: { keyword } }),

  // AI 기반 SNS 게시글 생성
  generatePost: (data) =>
    api.post(`${BASE_URL}-post/agent/post`, data, {
      headers: {
  ...
  generateHashtags: (data) =>
    api.post(`${BASE_URL}-post/agent/hashtags`, data, {
      headers: {
        "Content-Type": "application/json",
      },
    }),
