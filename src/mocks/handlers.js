...
export const handlers = [
  ...
  http.all("*", ({ request }) => {
    ...
    // /realtime/posts/report 엔드포인트만 MSW로 처리
    const isReportEndpoint = url.pathname.includes('/realtime/posts/report');
    ...
    // /realtime/posts/report가 아니면 백엔드로 통과
    if (!isReportEndpoint) {
  ...
  // AI 분석 보고서 조회 (캐시 포함)
  http.get(`${API_BASE_URL}/api/analytics/realtime/posts/report`, ({ request }) => {
    const url = new URL(request.url);
    const accountId = url.searchParams.get("accountId");
    const postId = url.searchParams.get("postId");
    ...
    if (!accountId || !postId) {
      return HttpResponse.json(
        {
          isSuccess: false,
          message: "accountId와 postId 파라미터가 필요합니다.",
          result: null,
        },
        { status: 400 }
      );
    ...
    // AI 서버에서 생성된 마크다운 보고서 (캐시된 데이터)
    const markdownReport = `
    # 디지털 마케팅 보고서

### 캠페인 분석: *여름철 뷰티 루틴 완벽 가이드*

---

## 1. 게시물 개요

* **주제:** 여름철 스킨케어 & 메이크업 팁 영상
* **지역성:** 서울 강남 기반 트렌디 뷰티 정보 강조
* **목적:** 시즌성 뷰티 관심도를 활용한 타겟 고객 유입
...
> **인사이트:** 조회수 대비 댓글 비율이 높음 → 참여도(Engagement)가 ‘의견 교환 중심’으로 활발히 일어나는 상황

---

## 3. 감정 분석 (Emotion Data)

* **긍정 반응:** 6건

  * 대표 키워드: *멋지네요, 좋아요, 맘에들어요, 가격 궁금해요*
* **부정 반응:** 2건
...
> **인사이트:**
>
> * 긍정 피드백 다수 → 브랜드 호감도 긍정적
> * “가격” 관련 질문 반복 → 구매 전환으로 이어질 가능성 높음
> * 부정 피드백은 ‘스타일·톤앤매너’ 관련 → 비주얼·콘셉트 조정 필요
...
1. **참여 패턴**
   ...
   * 단순 시청보다는 댓글 중심의 반응이 많아 **소통형 콘텐츠**로서 잠재력이 높음.
2. **전환 가능성**

   * “가격” 키워드 다수 출현 → 제품 소개 또는 상세 정보 제공 시 **매출 직결 가능성** 큼.
3. **브랜드 이미지 관리**

   * 일부 “촌스럽다”는 부정 반응은 **영상 연출·톤앤매너 개선**을 요구하는 신호.
...
* 댓글 내 “가격” 질문에 빠른 피드백 → 구매 링크 또는 상세 안내 제공
* 긍정적 댓글에 답글·하트 표시 → 충성도 강화

✅ **단기 개선 (1\~2주)**

* 영상 내 자막/CTA(Call To Action)에 **가격·구매 경로 명확히 노출**
* 댓글 반응을 바탕으로 *FAQ 콘텐츠* 제작

✅ **중장기 전략 (다음 캠페인 반영)**

* 영상 톤앤매너 개선: *트렌디·세련된 무드 보강*
* 강남 로컬 키워드 활용한 **지역 타겟 광고 집행**
* 긍정 키워드 활용 → “맘에 들어요” 기반 **리뷰형 UGC(User Generated Content) 캠페인** 기획

---

## 6. 핵심 메시지

> 이번 캠페인은 \*\*소규모 도달(조회수는 낮음)\*\*에도 불구하고
> \*\*높은 참여도(댓글 비율)\*\*와 \*\*구매 의향 신호(가격 관련 질문)\*\*가 나타난 것이 가장 큰 성과입니다.
> 빠른 고객 응대와 톤앤매너 개선을 통해 **전환 최적화 및 브랜드 이미지 제고**가 필요합니다.
