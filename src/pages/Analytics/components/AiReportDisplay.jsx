...
export const AiReportDisplay = ({ 
  ...
  const renderMarkdownContent = (markdownText) => {
    ...
    return (
      <div className="markdown-content prose prose-sm max-w-none prose-headings:text-gray-900 prose-p:text-gray-600 prose-li:text-gray-600 prose-strong:text-gray-900">
        <ReactMarkdown
          components={{
            h1: ({ children }) => (
  ...
  return (
    <div className={`ai-report-display ${className}`}>
      <div className="p-8">
        ...
        {report && !loading && (
          <div>
            <div className="bg-gradient-to-r from-purple-50 to-purple-100/50 rounded-xl p-4 mb-6 border border-purple-200/50">
              <div className="flex items-center space-x-2 text-sm text-purple-700">
                <Info className="text-purple-600" />
                <span className="font-medium">게시물 ID: {report.postId}</span>
                <span>•</span>
                <span>AI 분석 결과 (캐시된 데이터)</span>
              </div>
            </div>
